...
class EpiNet(object):
    ...
    def BuildLabeledCPM(self, batch_size):
        ...
        self.label = tf.placeholder(dtype=tf.float32, shape=(
        batch_size, self.label_size, self.label_size, self.nJoints + 1 + 2 * self.nLimbs))
    ...
    def BuildUnlabeledCPM(self, batch_size):
        ...
        self.heatmap_unlabeled = tf.placeholder(dtype=tf.float32,
                                                shape=(batch_size, self.label_size, self.label_size, self.nJoints + 1))
        ...
        self.model_ref.build_loss_unsupervised(self.pred0, self.heatmap_unlabeled, self.confident_joint,
                                               self.lr_unlabeled, self.lr_decay_rate, self.lr_decay_step)
...
class CPM_Model(object):
    ...
    def build_loss_unsupervised(self, gt_heatmap, heatmap_unlabeled, confident_joint, lr, lr_decay_rate, lr_decay_step):
        ...
        for stage in range(self.stages):
            ...
            unimodality = tf.reduce_max(self.stage_joint_heatmap[stage], axis=[1, 2]) - tf.reduce_min(
                self.stage_joint_heatmap[stage], axis=[1, 2])
            ...
            self.l2_loss[stage] = tf.nn.l2_loss(
                (self.stage_joint_heatmap[stage][:, :, :, :-1] - self.gt_heatmap[:, :, :, :-1])) / self.batch_size

            heatmap_unlabeled = tf.nn.l2_normalize(heatmap_unlabeled, [1, 2])

            self.l2_loss_unlabeled[stage] = tf.nn.l2_loss(
                confident_joint * (
                self.stage_joint_heatmap[stage][:, :, :, :-1] - heatmap_unlabeled[:, :, :, :-1])) / self.batch_size
    ...
    def build_loss(self, gt_heatmap, lr, lr_decay_rate, lr_decay_step):
        ...
        for stage in range(self.stages):
            ...
            self.stage_loss[stage] = tf.nn.l2_loss(
                tf.concat([self.stage_joint_heatmap[stage], self.stage_limb_heatmap[stage]],
                          3) - self.gt_heatmap) / self.batch_size
            # tf.summary.scalar('stage' + str(stage + 1) + '_loss', self.stage_loss[stage])
